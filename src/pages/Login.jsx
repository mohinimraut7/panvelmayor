    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
        ...
        /* Background image with light-yellow tinted overlay */
        .lp-bg {
          position:absolute; inset:0;
          background-image: url(${heroBg});
          background-size:cover; background-position:center;
        ...
        .lp-bg::after {
          content:''; position:absolute; inset:0;
          // background: rgba(218,187,74,0.90);
          // background: rgba(131,189,215,0.90);
        //  background: rgba(58,61,114,0.90);  
          background: linear-gradient(to right, rgba(58,61,114,0.90) 70%, rgba(219,184,69,0.90) 100%);  /* ← इथे */

        }

        .lp-orb { position:absolute; border-radius:50%; pointer-events:none; z-index:1; animation:orbFloat 8s ease-in-out infinite; }
        .lp-orb-1 { width:380px;height:380px;top:-120px;right:-100px;background:radial-gradient(circle,rgba(131,189,215,0.22) 0%,transparent 70%);animation-delay:0s; }
        .lp-orb-2 { width:280px;height:280px;bottom:-80px;left:-60px;background:radial-gradient(circle,rgba(252,242,166,0.15) 0%,transparent 70%);animation-delay:3s; }
        @keyframes orbFloat { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-20px)} }
        ...
        .lp-logo-ring img { width:100%;height:100%;border-radius:50%;object-fit:cover;border:2px solid #fff; }
        .lp-org-name { font-family:'Noto Sans Devanagari',sans-serif; font-size:14px;font-weight:800;color:var(--dark-blue);line-height:1.3; }
        .lp-org-sub  { font-size:11px;color:var(--sky-blue);font-weight:700;letter-spacing:0.4px; }

        /* Tabs */
        .lp-tabs {
          display:flex; background:rgba(58,61,114,0.08);
          border-radius:12px; padding:4px; margin-bottom:16px;
          border:1px solid rgba(58,61,114,0.12);
        }
        .lp-tab {
          flex:1; padding:9px 8px; border:none; border-radius:10px;
          font-size:13px;font-weight:600;cursor:pointer;transition:all 0.25s;
        ...
        .lp-tab.active {
          background:var(--dark-blue);
          color:var(--btn-text);
          box-shadow:0 3px 12px rgba(58,61,114,0.4);
        }
        ...
        /* Field */
        .lp-field { margin-bottom:14px; }
        .lp-label { display:block;font-size:11px;font-weight:700;color:var(--dark-blue);margin-bottom:5px;letter-spacing:0.4px;text-transform:uppercase; }
        .lp-input-wrap { position:relative; }
        .lp-icon { position:absolute;left:12px;top:50%;transform:translateY(-50%);font-size:15px;color:var(--dark-blue);opacity:0.65; }
        .lp-input {
          width:100%;padding:10px 12px 10px 40px;font-size:13.5px;
        ...
        .lp-input::placeholder { color:#b0b8cc;font-family:'Noto Sans Devanagari',sans-serif;font-size:12.5px; }
        .lp-pass-toggle { position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:16px;color:#8090a8;padding:0;transition:color 0.2s; }
        .lp-pass-toggle:hover { color:var(--dark-blue); }

        /* Button — dark blue bg, light yellow text */
        .lp-btn {
          width:100%;padding:12px;border-radius:11px;border:none;
          background:linear-gradient(135deg,var(--dark-blue) 0%,#4e52a0 100%);
          color:var(--btn-text);
          font-weight:700;font-size:14px;
          cursor:pointer;transition:all 0.25s;
          box-shadow:0 4px 18px rgba(58,61,114,0.4);
          font-family:'DM Sans',sans-serif;letter-spacing:0.3px;
          text-shadow:0 1px 4px rgba(0,0,0,0.18);
        }
        .lp-btn:hover:not(:disabled) {
          transform:translateY(-2px);
          box-shadow:0 8px 28px rgba(58,61,114,0.5);
          background:linear-gradient(135deg,#4e52a0 0%,var(--dark-blue) 100%);
        }
        .lp-btn:active:not(:disabled) { transform:translateY(0); }
        .lp-btn:disabled { background:#d1d5db;color:#9ca3af;cursor:not-allowed;box-shadow:none;transform:none;text-shadow:none; }

        /* Footer */
        .lp-footer { text-align:center;margin-top:14px;font-size:13px;color:#6b7280; }
        .lp-link { color:var(--dark-blue);font-weight:700;cursor:pointer;text-decoration:none;transition:color 0.2s; }
        .lp-link:hover { color:var(--sky-blue);text-decoration:underline; }

        /* OTP prefix */
        .lp-prefix { position:absolute;left:12px;top:50%;transform:translateY(-50%);font-size:13px;font-weight:700;color:var(--dark-blue);display:flex;align-items:center;gap:4px; }
        .lp-input.with-prefix { padding-left:78px; }

        /* OTP boxes */
        .lp-otp-boxes { display:flex;gap:7px;justify-content:center;margin-bottom:14px; }
        .lp-otp-box {
          width:43px;height:50px;border:1.5px solid rgba(58,61,114,0.2);border-radius:11px;
          font-size:22px;font-weight:800;text-align:center;color:var(--dark-blue);
          outline:none;background:#fff;transition:all 0.2s;font-family:'Crimson Pro',serif;
        }
        .lp-otp-box:focus {
          border-color:var(--sky-blue);
          box-shadow:0 0 0 3px rgba(131,189,215,0.25);
          background:#f0f8fd;
        }
        ...
        .lp-timer { text-align:center;font-size:13px;color:#6b7280;margin-bottom:14px; }
        .lp-back-btn { background:none;border:none;font-size:13px;font-weight:600;color:#6b7280;cursor:pointer;display:flex;align-items:center;gap:4px;margin-bottom:12px;padding:0;font-family:'DM Sans',sans-serif;transition:color 0.2s; }
        .lp-back-btn:hover { color:var(--dark-blue); }
        .lp-resend { background:none;border:none;font-size:13px;font-weight:700;cursor:pointer;transition:all 0.2s;font-family:'DM Sans',sans-serif; }
        .lp-resend:disabled { color:#94a3b8;cursor:not-allowed; }
        .lp-resend:not(:disabled) { color:var(--sky-blue); }
        .lp-resend:not(:disabled):hover { text-decoration:underline;color:var(--dark-blue); }

        /* ── RIGHT PANEL — sky blue background ── */
        .lp-right {
          width:310px;flex-shrink:0;
          // background:linear-gradient(160deg,var(--sky-blue) 0%,#5fa8c8 40%,#3d8db0 100%);
          background:linear-gradient(160deg, #3A3D72 0%, #4a4e8f 40%, #2e3160 100%);  /* ← इथे */
          padding:40px 28px;
          display:flex;flex-direction:column;align-items:center;justify-content:center;
          position:relative;overflow:hidden;
        }
        ...
        .lp-right-deco::before {
          content:'';position:absolute;top:-80px;right:-80px;width:280px;height:280px;border-radius:50%;
          background:radial-gradient(circle,rgba(252,242,166,0.2) 0%,transparent 60%);
        }
        .lp-right-deco::after {
          content:'';position:absolute;bottom:-60px;left:-60px;width:200px;height:200px;border-radius:50%;
        ...
        /* Chevrons */
        .lp-chevrons { position:absolute;top:18px;right:12px;opacity:0.18;pointer-events:none; }

        /* Floating icons */
        .lp-float { position:absolute;font-size:38px;opacity:0.1;color:#fff;animation:iconDrift 6s ease-in-out infinite;pointer-events:none; }
        .lp-float-1{top:10%;left:8%;animation-delay:0s}
        .lp-float-2{top:20%;right:10%;animation-delay:1.5s}
        .lp-float-3{bottom:22%;left:6%;animation-delay:3s}
        .lp-float-4{bottom:10%;right:8%;animation-delay:4.5s}
        ...
        .lp-mayor-ring {
          ...
          box-shadow:
            0 0 0 5px rgba(252,242,166,0.2),
        ...
        .lp-quote {
          ...
          border:1px solid rgba(252,242,166,0.3);
          border-radius:14px;padding:14px 16px;text-align:center;
          backdrop-filter:blur(4px);
        }
        .lp-quote-mark {
          font-family:'Crimson Pro',serif;font-size:26px;
          color:var(--btn-text);
          line-height:0.5;display:block;margin-bottom:8px;opacity:0.85;
        }
        .lp-quote-text {
          font-family:'Noto Sans Devanagari',sans-serif;
          font-size:12.5px;color:rgba(255,255,255,0.92);line-height:1.75;
        }

        /* ── RESPONSIVE ── */
        @media(max-width:768px){
          .lp-card{flex-direction:column-reverse; }
          .lp-right{width:100%;padding:32px 24px}
          .lp-mayor-ring{width:120px;height:120px}
          .lp-left{padding:24px 20px}
        }
              ...
              <div>
                ...
                {otpStep === "otp" && (
                  <>
                    ...
                    <div style={{ fontSize:13, color:"#64748b", marginBottom:12 }}>
                      <span style={{ color:"var(--sky-blue)", fontWeight:700 }}>+91 ******{mobileNo.slice(-3)}</span> वर OTP पाठवला
                    </div>
                    <div className="lp-otp-boxes" onPaste={handleOtpPaste}>
                      {otp.map((digit, i) => (
                        <input key={i} ref={(el) => (otpRefs.current[i] = el)}
                    ...
                    <div className="lp-timer">
                      {timeLeft > 0 ? (
                        <>OTP expire होईल: <strong style={{ color: timeLeft <= 15 ? "#dc2626" : "var(--sky-blue)" }}>{formatTime(timeLeft)}</strong></>
                      ) : (
                        <span style={{ color:"#dc2626", fontWeight:600 }}>OTP expire झाला!</span>
                      )}
                      <div style={{ marginTop:5 }}>
